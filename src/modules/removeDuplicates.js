...
const removeDuplicates = (arr, multi_dim = true, truthy_only = true, sort_by = 'none') => {
    ...
    if (multi_dim === true) {
        ...
        throw new MethodException(`Illegal use of parameter multi_dim - Expected true or false - Found: ${multi_dim}`)
    }
    ...
    if (truthy_only === true) {
        newArray = newArray.filter((e) => {
            ...
            } else if (!e && e !== 0) { // false, null or undefined NOT 0 (number) ref#1
                return false
            } else if (typeof e === "boolean") { // Boolean: True or False
                return false
            } else if (typeof e === "object") { // Object or null (can't be null because ref#1 was checked)
                if (Object.keys(e).length > 0) return true // Object not empty
                return false // Empty object
            } else if (typeof e === "function") { // is function
                return false
        ...
        throw new MethodException(`Illegal use of parameter multi_dim  - Expected true or false - Found: ${truthy_only}`)
    }
    ...
    if (Array.isArray(sort_by)) {
        ...
        for (const method in methodsHashMap) {
            if (methodsHashMap[method] > 1) { throw new MethodException(`Illegal use of parameter sort_by - Cannot use a method more than once - Duplicate methods : (${method})`) }
        }
    ...
    } else if (sort_by === "none") { } // Okay
    else {
        throw new MethodException(`Illegal use of parameter sort_by - Expected "none" or ["alphabetic","numeric","lengthwise"] - Found: ${sort_by}`)
    }
